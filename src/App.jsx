...
import { newfiberAuth, newfiberfbProvider } from './facebook/faceFirebase';
...
const App = () => {
  ...
  const showPages = async () => {
    try {
      ...
      newfiberfbProvider.addScope("pages_show_list");
      newfiberfbProvider.addScope("pages_read_engagement");
      newfiberfbProvider.addScope("pages_manage_posts");
      newfiberfbProvider.addScope("instagram_basic");
      newfiberfbProvider.addScope("instagram_content_publish");


      console.log("Work starting...");

      const myResult = await signInWithPopup(newfiberAuth, newfiberfbProvider);
      console.log("my result:", myResult);


      const secret = FacebookAuthProvider.credentialFromResult(myResult);

      console.log("my seceret", secret);
